...
def plot_one_box0(x, img, color=None, label=None, line_thickness=None):
    """
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        text_org = ((c1[0] + c2[0]) // 2 - t_size[0] // 2, (c1[1] + c2[1]) // 2 + t_size[1] // 2)
        
        cv2.rectangle(
            img,
            (text_org[0] - 2, text_org[1] - t_size[1] - 2),
            (text_org[0] + t_size[0] + 2, text_org[1] + 2),
            color,
            -1,
            cv2.LINE_AA,
        )  # filled
...
if __name__ == "__main__":
    ...
    model_pb_path = r"/home/pi/Desktop/sleep/file/model/3.onnx"
    so = ort.SessionOptions()
    net = ort.InferenceSession(model_pb_path, so)
    
    # 标签字典
    dic_labels= {0:'book',1:'phone'}
